      ...
      <div className="px-4 py-2 space-y-4">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          {categories.map((cat, i) => (
            <button
              key={cat}
              className={`flex-shrink-0 px-5 py-2 rounded-full text-sm font-bold transition-all ${i === 0 ? "bg-primary text-white neon-glow" : "bg-secondary text-muted-foreground"
                }`}
            >
              {cat}
            </button>
